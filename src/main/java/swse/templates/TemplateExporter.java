                ...
                switch (toks[0]){
                    ...
                    case "Shield Rating (SR)":
                        if(toks[1].equals("Any time the SR value of an Archaic Vehicle is exceeded, there is a 50% chance they fail entirely. They may be recharged normally (Using the Recharge Shields Action).")){
                            response.add(Change.create(AttributeKey.SHIELD_FAILURE_CHANCE, "50%"));
                    ...
                    case "Hit Points":
                        if(toks[1].equals("Increase the ship's total Hit Points by 25% (Rounded up).")){
                            response.add(Change.create(AttributeKey.HIT_POINT_EQ, "*1.5"));
                            ...
                            response.add(Change.create(AttributeKey.LIVING_VEHICLE, "true"));
                        }
                        break;
                    case "Strength":
    ...
    private static void getChallengeLevel(List<Object> response, String[] toks) {
        switch(toks[1]){
            case "The CL of an Advanced Vehicle is 125% of it's normal value (Rounded up).":
                response.add(Change.create(AttributeKey.CHALLENGE_LEVEL, "*1.25"));
                break;
            case "The CL of an Archaic Vehicle is 2/3 it's normal value (Rounded up).":
                response.add(Change.create(AttributeKey.CHALLENGE_LEVEL, "*2 /3"));
                break;
            case "Reduce the CL of a Junker by 20% (Rounded down).":
                response.add(Change.create(AttributeKey.CHALLENGE_LEVEL, "*0.8"));
                break;
            case "Reduce the base Vehicle's CL by -2.":
                response.add(Change.create(AttributeKey.CHALLENGE_LEVEL, "-2"));
                break;
    ...
    private static Collection<?> getManualAttributes(String resolvedName) {
        ...
        switch(resolvedName){
            case "Advanced Vehicle Template":
                response.add(Change.create(AttributeKey.CHALLENGE_LEVEL, "+25%"));
                ...
                response.add(Change.create(AttributeKey.DAMAGE_BONUS_VEHICLE, 1));

                break;
            case "Archaic Vehicle Template":
                response.add(Change.create(AttributeKey.DAMAGE_THRESHOLD_BONUS, "-50%"));
                response.add(Change.create(AttributeKey.SHIELD_FAILURE_CHANCE, "50%"));
                response.add(Change.create(AttributeKey.CRITICAL_HITS, false));
                response.add(Change.create(AttributeKey.CRITICAL_HITS, true).withModifier("Archaic Vehicles"));
                response.add(Change.create(AttributeKey.ATTACK_BONUS_VEHICLE, -2));
                break;
            case "Junker Vehicle Template":
                response.add(Change.create(AttributeKey.INITIATIVE_BONUS, "-5"));
                response.add(ProvidedItem.create("Intelligence (-4)", ItemType.TRAIT));
            ...
            case "Prototype Vehicle Template":
                ...
                response.add(Change.create(AttributeKey.SPEED_STARSHIP_SCALE, 1));
            ...
            case "Antiquated Vehicle Template":
                ...
                response.add(Change.create(AttributeKey.AVAILABILITY, "Rare"));
                response.add(Change.create(AttributeKey.COST, "(USED OR 60%)"));
                response.add(Change.create(AttributeKey.SPECIAL, "Repairs: Antiquated Vehicles do not use modern technology and are likely to be unfamiliar to most ship engineers. The base DC of any Mechanics checks made to Build Object or Repair increase by +5 when associated with an Antiquated Vehicle."));
                response.add(Change.create(AttributeKey.SPECIAL, "Refitting: An Antiquated Vehicle can be Refitted. Apply the Refitted Vehicle Template to the base Vehicle instead of the Antiquated Vehicle Template."));
                break;
            case "Living Vehicle Template": break;
            case "Refitted Vehicle Template":
                response.add(Change.create(AttributeKey.SPECIAL, "Repairs: Since Refitted Vehicles combine antiquated technology with modern components, sometimes the Vehicle must be treated gently and repairs can be challenging. A Refitted Vehicle increases the base DC of any Mechanics checks made to Repair it by +5."));
                response.add(Change.create(AttributeKey.SPECIAL, "Maintenance: Unlike normal Vehicles, Refitted Vehicles must be given extra maintenance to ensure that they continue to function as expected. Once per week the Refitted Vehicle must receive regular maintenance, which requires 1 hour of work (no Mechanics check required, though a Tool Kit is necessary to perform the maintenance).\n" +
                ...
                response.add(Change.create(AttributeKey.AVAILABILITY, "Rare"));
                response.add(Change.create(AttributeKey.COST, "(USED + 20%)"));
                break;
            case "Baragwin Weapon Template":
                response.add(Change.create(AttributeKey.CRITICAL_HIT_POSTMULTIPLIER_BONUS_DIE, 1));
                response.add(Change.create(AttributeKey.AVAILABILITY, "Illegal"));
                break;
            case "Bothan Weapon Template":
                response.add(Change.create(AttributeKey.BONUS_DAMAGE_DIE, "1").withPrerequisite(or(simple("damage type: ion", "DAMAGE_TYPE", "Ion"), simple("damage type: stun", "DAMAGE_TYPE", "Stun"))));
                response.add(Change.create(AttributeKey.BONUS_DAMAGE_DIE, "-1").withPrerequisite(not(or(simple("damage type: ion", "DAMAGE_TYPE", "Ion"), simple("damage type: stun", "DAMAGE_TYPE", "Stun")))));
                break;
            case "Dashade Weapon Template":
                response.add(Change.create(AttributeKey.SPECIAL, "A target moved down the Condition Track by an attack from a Dashade Manufacture Weapon can only spend 2 Swift Actions per turn to Recover. This effect lasts until the target return to its normal state (all conditions removed)."));
                response.add(Change.create(AttributeKey.BONUS_DAMAGE, "-2"));
                break;
            case "Gand Weapon Template":
                response.add(Change.create(AttributeKey.SPECIAL, "If a Gand Manufacture Weapon moves a target down the Condition Track with it's Stun damage, that target has it's speed halved until it returns to its normal state (all conditions removed); a target that is -4 steps down the Condition Track (at the -10 step) is Immobilized instead."));
                response.add(Change.create(AttributeKey.BONUS_DAMAGE, "-2"));
                response.add(Change.create(AttributeKey.EXOTIC_WEAPON, "Gand"));
                break;
            case "Quick Draw Weapon Template":
                response.add(Change.create(AttributeKey.SPECIAL, "A Quick Draw Weapon wielder that possesses the Quick Draw feat can draw or holster the weapon once per round as a Free Action."));
                break;
            case "Rakatan Weapon Template":
                response.add(Change.create(AttributeKey.BONUS_DAMAGE_DIE_TYPE, "1"));
                response.add(Change.create(AttributeKey.TO_HIT_MODIFIER, "1"));
                response.add(Change.create(AttributeKey.EXOTIC_WEAPON, "Rakatan"));
                break;
            case "Sith Alchemical Weapon Template":
                response.add(Change.create(AttributeKey.BLOCKS_LIGHTSABER, "true"));
                response.add(Change.create(AttributeKey.ALLOWS_BLOCK, "true"));
                response.add(Change.create(AttributeKey.ALLOWS_DEFLECT, "true"));
                response.add(Change.create(AttributeKey.ALLOWS_REDIRECT_SHOT, "true"));
                response.add(Change.create(AttributeKey.SPECIAL, "A character proficient with a Sith Alchemical Weapon can spend a Force Point as a Swift Action to gain a bonus to damage rolls with the Sith Alchemical Weapon equal to his or her Dark Side Score. This bonus to damage applies to the next attack made with the Sith Alchemical Weapon before the end of the encounter, and activating this ability increases the wielder's Dark Side Score by 1."));
                break;
            case "Antiquated Weapon Template":
                response.add(Change.create(AttributeKey.SPECIAL, "Antiquated Energy Cells and Antiquated Power Packs for Antiquated Weapons cost 50% more than a standard Energy Cell or Power Pack. Unless otherwise specified, an Antiquated Weapon cannot use standard Energy Cells or Power Packs, only Antiquated Energy Cells or Antiquated Power Packs."));
                response.add(Change.create(AttributeKey.SPECIAL, "Antiquated Weapons do not use modern technology and are likely to be unfamiliar to most weaponsmiths. An Antiquated Weapon increases the base DC of any Mechanics check made to build or repair the Weapon by +5."));
                response.add(Change.create(AttributeKey.SPECIAL, "An Antiquated Weapon can be Refitted. Apply the Refitted Weapon Template to the base Weapon instead of the Antiquated Weapon Template."));
                Change change = Change.create(AttributeKey.AVAILABILITY, "Rare");
                change.withMode(ActiveEffectMode.OVERRIDE);
                response.add(change);
                break;
            case "Refitted Weapon Template":
                response.add(Change.create(AttributeKey.SPECIAL, "A Refitted Weapon can use standard Energy Cells and Power Packs. Additionally, weapons that use Power Packs to provide ammunition have their number of shots that can be fired before the Power Pack must be replaced increased by 10%."));
                response.add(Change.create(AttributeKey.SPECIAL, "Since Refitted Weapons combine antiquated technology with modern components, sometimes the weapon must be treated gently and repairs can be challenging. A Refitted Weapon increases the base DC of any Mechanics check made to build or repair the Weapon by +5."));
                response.add(Change.create(AttributeKey.SPECIAL, "Unlike normal weapons, Refitted Weapons require extra maintenance to ensure that they continue to function as expected. Once per week the weapon must receive regular maintenance, which requires 1 hour or work (no Mechanics check required, though a Tool Kit is necessary to perform the maintenance).\n" +
                        "\n" +
            ...
            case "Sawed-Off Weapon Template":
                response.add(Change.create(AttributeKey.SIZE_BONUS, "-1"));
                response.add(Change.create(AttributeKey.RANGE_MULTIPLIER, "/2"));
                break;
            case "Mandalorian Armor Template":
                response.add(Change.create(AttributeKey.FORTITUDE_DEFENSE_BONUS_EQUIPMENT, "+1"));
                response.add(Change.create(AttributeKey.SPECIAL, "Additionally, if the wearer has Damage Reduction, they can apply its benefit against attacks made by Lightsabers."));
                response.add(Change.create(AttributeKey.SPECIAL, "In some eras, particularly during The Mandalorian Wars, the armor alone grants favorable circumstances on Persuasion checks made to Intimidate."));

                break;
            case "Bonadan-Alloy Armor Template":
                response.add(Change.create(AttributeKey.DAMAGE_REDUCTION, "2").withModifier("Slashing"));
                response.add(Change.create(AttributeKey.FORTITUDE_DEFENSE_BONUS_EQUIPMENT, "-1"));
                break;
            case "Bronzium Cast Armor Template":
                response.add(Change.create(AttributeKey.SPECIAL, "Once per encounter, the wearer of a suit of Bronzium Cast Armor can redirect the damage from a successful incoming attack to the armor; the Bronzium Cast Armor takes the damage from the attack (armor has the same DR, Hit Points, Damage Threshold, and break DC as a weapon of the same size category as the wearer (see Attack an Object)).\n" +
                        "\n" +
            ...
            case "Durasteel Cast Armor Template":
                response.add(Change.create(AttributeKey.DAMAGE_REDUCTION, "1"));
                response.add(Change.create(AttributeKey.FORTITUDE_DEFENSE_BONUS_EQUIPMENT, "-1"));
                break;
            case "Environmental Bastion Armor Template":
                response.add(Change.create(AttributeKey.SPECIAL, "Environmental Bastion Armor acts as a sealed Space Suit, providing a number of hours of breathable air to the wielder equal to the armor's Equipment bonus to Fortitude Defense."));
                break;
            case "Eriadun Armor Template":
                response.add(Change.create(AttributeKey.DAMAGE_REDUCTION, "5").withModifier("Stun"));
                response.add(Change.create(AttributeKey.FORTITUDE_DEFENSE_BONUS_EQUIPMENT, "-1"));
                break;
            case "Stygian-Triprismatic Polymer Armor Template":
                response.add(Change.create(AttributeKey.REFLEX_DEFENSE_BONUS_ARMOR, "+1"));
                response.add(Change.create(AttributeKey.SKILL_BONUS, "stealth:2").withModifier("electronic sensors"));
                break;
            case "Advanced Droid Template":
                response.add(Change.create(AttributeKey.BONUS_SPECIAL_TRAIT, 1));
                response.add(Change.create(AttributeKey.DAMAGE_THRESHOLD_BONUS, "5"));
                response.add(Change.create(AttributeKey.SPEED_BONUS, "1"));
                response.add(Change.create(AttributeKey.COST, "*2"));
                response.add(Change.create(AttributeKey.SPECIAL, "The repair and maintenance costs associated with an Advanced Droid are doubled, and any repair DCs are increased by 5."));
                break;
            case "Archaic Droid Template":
                response.add(Change.create(AttributeKey.DAMAGE_THRESHOLD_BONUS, "-5"));
                response.add(Change.create(AttributeKey.COST, "/2"));
                response.add(Change.create(AttributeKey.SPECIAL, "The DC for all Mechanics and Use Computer checks used to repair the Archaic Droid are increased by 10. Additionally, any built-in weaponry on an Archaic Droid is incapable of scoring a Critical Hit against non-archaic Droids and equipment."));
                break;
            case "Converted Droid Template":
                response.add(Change.create(AttributeKey.BONUS_DROID_QUIRK, 1));
                response.add(Change.create(AttributeKey.SPECIAL, "The Droid is treated as being the Degree of the Droid Chassis its been converted to, and it gains the bonus equipment, Ability Modifiers, and bonus Feats associated with the Droid Chassis."));
                break;
            case "Junker Droid Template":
                response.add(Change.create(AttributeKey.DAMAGE_THRESHOLD_BONUS, "-5"));
                response.add(Change.create(AttributeKey.SPECIAL, "must develop a quirk (See Droid Quirks) each time the Droid takes damage that exceeds its Damage Threshold. The DC of the skill check associated with repairing that quirk is also reduced by 5, and the repair costs are reduced by 50%."));
                break;
            case "Arkanian General Template (weapon)":
                response.add(Change.create(AttributeKey.DAMAGE_TYPE, "Fire"));
                response.add(Change.create(AttributeKey.OVERHEAT_LIMIT, "1"));
                response.add(Change.create(AttributeKey.COOLDOWN_TIME, "1"));
                response.add(Change.create(AttributeKey.SPECIAL, "If this Weapon is used with an ability that consumes more than one shot in a round (such as Rapid Shot or Double Attack), the weapon overheats and cannot be fired for one round as it cools down."));
                break;
            case "Arkanian General Template (armor)":
                response.add(Change.create(AttributeKey.FORTITUDE_DEFENSE_BONUS_EQUIPMENT, "x2").withModifier("Extreme Cold"));
                response.add(Change.create(AttributeKey.FORTITUDE_DEFENSE_BONUS_EQUIPMENT, "x0").withModifier("Extreme Heat"));
            break;
            case "Cinnagaran General Template (weapon)":
                response.add(Change.create(AttributeKey.DAMAGE_TYPE, "Bludgeoning"));
                break;
            case "Cinnagaran General Template (armor)":
                response.add(Change.create(AttributeKey.DAMAGE_REDUCTION, "5").withModifier("Sonic"));
                response.add(Change.create(AttributeKey.MAXIMUM_DEXTERITY_MODIFIER, "-1"));
                break;
            ...
            case "Phrik Alloy General Template (armor)":
                response.add(Change.create(AttributeKey.BLOCKS_LIGHTSABER, "true"));
                response.add(Change.create(AttributeKey.AVAILABILITY, "Rare").withModifier("EXCEPT:ERA:Old Republic Era")); //TODO disable for old republic era
                break;
            case "Echani General Template (weapon)": 
                response.add(Change.create(AttributeKey.ACTION, "Once per encounter the wielder of an Echani Manufacture Weapon can declare that the critical range of their weapon is increased by one (thus scoring a Critical Hit on a Natural 19 or Natural 20 on most weapons). The use of this ability can be declared after the attack roll is made, but before the attack is resolved."));
                response.add(Change.create(AttributeKey.HP_WEAPON, "*0.5"));
                response.add(Effect.create("Extended Critical Range", List.of(Change.create(AttributeKey.EXTENDED_CRITICAL_HIT, 19))));
                break;
            case "Echani General Template (armor)":
                response.add(Change.create(AttributeKey.ACTION, "once per encounter the wearer can move at their normal speed (not the adjusted speed for wearing the Armor) for one round"));
                response.add(Change.create(AttributeKey.REFLEX_DEFENSE_BONUS_ARMOR, -1));
                break;
            case "GenoHaradan General Template (weapon)":
                response.add(Change.create(AttributeKey.TO_HIT_MODIFIER, 2).withPrerequisite(simple("damage type: stun", "DAMAGE_TYPE", "Contact Poison")));
                response.add(Change.create(AttributeKey.TO_HIT_MODIFIER, 1).withPrerequisite(simple("damage type: stun", "DAMAGE_TYPE", "Stun")));
                response.add(Change.create(AttributeKey.FRAGILE, "true"));
                break;
            case "GenoHaradan General Template (armor)":
                response.add(Change.create(AttributeKey.SKILL_BONUS, "stealth:2"));
                response.add(Change.create(AttributeKey.FORTITUDE_DEFENSE_BONUS_EQUIPMENT, -1));
                break;
            case "Iridonian General Template (weapon)":
                response.add(Change.create(AttributeKey.POWER_ATTACK_BONUS_DAMAGE, 2));
                response.add(Change.create(AttributeKey.POWER_BLAST_BONUS_DAMAGE, 2));
                response.add(Change.create(AttributeKey.AVAILABILITY, "Illegal"));
                break;
            case "Iridonian General Template (armor)":
                response.add(Change.create(AttributeKey.DAMAGE_REDUCTION, "2").withModifier("Bludgeoning"));
                response.add(Change.create(AttributeKey.AVAILABILITY, "Illegal"));
                break;
            case "Krath General Template (weapon)": 
                response.add(Change.create(AttributeKey.DARKSIDE_TAINT, 1));
                //TODO figure out status system in foundry
                response.add(Change.create(AttributeKey.SPECIAL, "If an attack roll with a Krath Manufacture Weapon equals or exceeds the target's Fortitude Defense as well as its Reflex Defense, the target takes 1d4 damage at the beginning of it's next turn."));
                break;
            case "Krath General Template (armor)":
                response.add(Change.create(AttributeKey.DARKSIDE_TAINT, 1));
                response.add(Change.create(AttributeKey.DAMAGE_REDUCTION, "2").withModifier("Energy"));
                break;
            case "Mandalorian General Template":
                response.add(Change.create(AttributeKey.SPECIAL, "Any character making a Mechanics check to repair or modify a Mandalorian Weapon or Mandalorian Armor gains a +5 Equipment bonus on the check."));
                response.add(Change.create(AttributeKey.SPECIAL, "If a Mandalorian Manufacture item is disabled, all of the Weapon and Armor Accessories to that item are destroyed (and must be purchased anew, not merely repaired)."));
                break;
            case "Massassi General Template (weapon)":
                response.add(Change.create(AttributeKey.CRITICAL_HIT_PREMULTIPLIER_BONUS, "@STRMOD * 2"));
                //response.add(Attribute.create(AttributeKey.TO_HIT_MODIFIER, "-2"));
                response.add(Change.create(AttributeKey.TO_HIT_MODIFIER, "-2").withModifier("Massassi General Template (weapon)").withParentPrerequisite(not(Prerequisite.create("Strength 15"))).withPrerequisite(simple("Medium", "WEAPON_SIZE", "Medium")));
                response.add(Change.create(AttributeKey.TO_HIT_MODIFIER, "-2").withModifier("Massassi General Template (weapon)").withParentPrerequisite(not(Prerequisite.create("Strength 17"))).withPrerequisite(simple("Large", "WEAPON_SIZE", "Large")));
                response.add(Change.create(AttributeKey.TO_HIT_MODIFIER, "-2").withModifier("Massassi General Template (weapon)").withParentPrerequisite(not(Prerequisite.create("Strength 19"))).withPrerequisite(simple("Huge", "WEAPON_SIZE", "Huge")));
                break;
            case "Massassi General Template (armor)":
                response.add(Change.create(AttributeKey.ACTION, "once per encounter the wearer can move at their normal speed (not the adjusted speed for wearing the Armor) for one round"));

                response.add(Change.create(AttributeKey.ARMOR_CHECK_PENALTY_OVERRIDE, "true").withModifier("Massassi General Template (armor)").withParentPrerequisite(not(Prerequisite.create("Strength 13"))).withPrerequisite(simple("Light Armor", "ARMOR_TYPE", "Light")));
                response.add(Change.create(AttributeKey.ARMOR_CHECK_PENALTY_OVERRIDE, "true").withModifier("Massassi General Template (armor)").withParentPrerequisite(not(Prerequisite.create("Strength 15"))).withPrerequisite(simple("Medium Armor", "ARMOR_TYPE", "Medium")));
                response.add(Change.create(AttributeKey.ARMOR_CHECK_PENALTY_OVERRIDE, "true").withModifier("Massassi General Template (armor)").withParentPrerequisite(not(Prerequisite.create("Strength 17"))).withPrerequisite(simple("Heavy Armor", "ARMOR_TYPE", "Heavy")));
                break;
            case "Prototype General Template (droid)":
                response.add(Change.create(AttributeKey.BONUS_SPECIAL_TRAIT, 1));
                response.add(Change.create(AttributeKey.BONUS_DROID_QUIRK, 1));
                response.add(Change.create(AttributeKey.DAMAGE_THRESHOLD_BONUS, "*0.75"));
                break;
            case "Prototype General Template (weapon)":
                response.add(Change.create(AttributeKey.BONUS_SPECIAL_TRAIT, 1));
                response.add(Change.create(AttributeKey.EXTENDED_CRITICAL_FAILURE, 2));
                response.add(Change.create(AttributeKey.EXTENDED_CRITICAL_FAILURE, 3));
                response.add(Change.create(AttributeKey.EXTENDED_CRITICAL_FAILURE, 4));

                break;
            case "Prototype General Template (armor)":
                response.add(Change.create(AttributeKey.BONUS_SPECIAL_TRAIT, 1));
                response.add(Change.create(AttributeKey.SPECIAL, "A wearer of Prototype Armor takes 1 more die of damage from a Critical Hit (before multiplying)"));

                break;
            case "Verpine General Template (weapon)":
                response.add(Effect.create("Ion", List.of(
                        Change.create(AttributeKey.DAMAGE_TYPE, "Ion")
                )));
                response.add(Change.create(AttributeKey.TO_HIT_MODIFIER, "-2").withParentPrerequisite(not(Prerequisite.create("Intelligence 13"))));

                break;
            case "Verpine General Template (armor)":
                response.add(Change.create(AttributeKey.ARMOR_CHECK_PENALTY_OVERRIDE, "true").withParentPrerequisite(not(Prerequisite.create("Dexterity 13"))));
                response.add(Change.create(AttributeKey.DAMAGE_REDUCTION, "10").withModifier("Ion"));
                break;
