...
public class BackgroundExporter extends BaseExporter {
    public static final String JSON_OUTPUT = "G:\\FoundryVTT\\Data\\systems\\swse\\raw_export\\background.json";
...
//                        .startsWith("Bonus Language")).map(Node::toString).collect(Collectors.joining());
    ...
    private Collection<?> getAttributeFromName(String itemName, String subtype) {
        ...
        switch (itemName) {
            case "Enslaved":
                response.add(Attribute.create(AttributeKey.GRAPPLE_BONUS, 2));
                break;
            case "Exiled":
    ...
    private Collection<?> getAttributeFromSecondColumn(String secondColumn, String subtype) {
        ...
        if ("event".equals(subtype)) {
            response.add(Attribute.create(AttributeKey.SPECIAL, secondColumn));
        }
        return response;
    }
    ...
    private Collection<?> getSkillChoice(Element content, String subtype) {
        ...
        for (String skill : skills) {
            choice.withOption(skill, new Option().withAttribute(Attribute.create(AttributeKey.CLASS_SKILL, skill)));
        }
        ...
        if ("occupation".equals(subtype)) {
            for (String skill : skills) {
                response.add(Attribute.create(AttributeKey.UNTRAINED_SKILL_BONUS, skill).withModifier("2"));
            }
        }
        return response;
    }
